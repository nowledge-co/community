...
| **OpenCode** | Running as OpenCode agent; `~/.config/opencode/` or `.opencode/` exists | Check `opencode --version`. **1.x:** `opencode plugin opencode-nowledge-mem@0.3.10 --global`. **2.x:** `opencode plugin add opencode-nowledge-mem` (global by default). Restart OpenCode so its hooks load | [Guide](https://mem.nowledge.co/docs/integrations/opencode) |
