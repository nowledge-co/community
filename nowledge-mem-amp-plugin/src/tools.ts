 ...
 *
 * This module owns the static description of the ten `nowledge_mem_*` tools
...
/**
 * Builds the ten tool execute handlers bound to the given dependencies.
 *
...
export function createToolExecutors(
  ...
  return {
    ...
    async nowledge_mem_save(input): Promise<ToolExecuteResult> {
      ...
      if (importance !== undefined) cmd.push("-i", String(Math.min(1, Math.max(0, importance))))
      return deps.nmem(cmd)
    },
