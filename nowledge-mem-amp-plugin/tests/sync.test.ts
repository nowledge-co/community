...
/** Fake timer pair that records handles. */
function fakeTimers(): { setTimer: SessionSyncManagerOptions["setTimer"]; clearTimer: SessionSyncManagerOptions["clearTimer"]; handles: number[] } {
  let counter = 0
  ...
  return {
    ...
    setTimer: (handler) => {
      ...
      // Fire immediately so tests can observe synchronous scheduling effects.
      handler()
...
describe("SessionSyncManager.scheduleSync", () => {
  ...
  it("schedules a capture that persists the thread", async () => {
    const nmemApi = fakeNmemApi({ "/threads": () => ({ ok: true, status: 200, data: {} }) })
    const manager = new SessionSyncManager(managerOptions({ nmemApi }))
    // The fake timer fires synchronously, starting an async capture; flush it.
    manager.scheduleSync(THREAD_ID)
  ...
  it("coalesces overlapping captures via the pending flag", async () => {
    ...
    // While the first is in flight, schedule an auto-sync (would set pending).
    manager.scheduleSync(THREAD_ID)
    ...
    // At least the initial create ran; dedup prevents further persistence of the same signature.
    expect(nmemApi.calls.length).toBeGreaterThanOrEqual(1)
  })
})
