...
**nowledge_mem_save** — Capture structured knowledge with type classification.
...
Type: decision
Importance: 0.8
→ Saved: Task events database choice [decision] (id: mem_abc)
```

Eight memory types: `fact`, `preference`, `decision`, `plan`, `procedure`, `learning`, `context`, `event` — each becomes a typed node in the knowledge graph.
