...
function isBlockedSpecialUseIpv4Address(address) {
...
function isPrivateIpAddress(address) {
	...
	if (strictIp) {
		...
		if (embeddedIpv4) return isBlockedSpecialUseIpv4Address(embeddedIpv4);
		return false;
	}
...
function assertAllowedHostOrIpOrThrow(hostnameOrIp) {
	if (isBlockedHostnameOrIp(hostnameOrIp)) throw new SsrFBlockedError(BLOCKED_HOST_OR_IP_MESSAGE);
}
function assertAllowedResolvedAddressesOrThrow(results) {
	for (const entry of results) if (isBlockedHostnameOrIp(entry.address)) throw new SsrFBlockedError(BLOCKED_RESOLVED_IP_MESSAGE);
}
...
async function resolvePinnedHostnameWithPolicy(hostname, params = {}) {
	...
	if (!skipPrivateNetworkChecks) assertAllowedHostOrIpOrThrow(normalized);
	...
	if (!skipPrivateNetworkChecks) assertAllowedResolvedAddressesOrThrow(results);
