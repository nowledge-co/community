...
import { D as isPlainObject$1, d as isRecord, g as resolveConfigDir, y as resolveUserPath } from "./utils-DCtXtPui.js";
import { T as normalizeChatChannelId, _ as CHANNEL_IDS } from "./subsystem-CYc2uK4P.js";
import { s as normalizeAgentId, t as DEFAULT_AGENT_ID } from "./session-key-D7AdMgCp.js";
import { c as resolveAgentWorkspaceDir, l as resolveDefaultAgentId } from "./agent-scope-CJ4YDowp.js";
import { Kt as DEFAULT_CONTEXT_TOKENS, et as loadShellEnvFallback, l as normalizeProviderId, nt as shouldDeferShellEnvFallback, rt as shouldEnableShellEnvFallback, tt as resolveShellEnvFallbackTimeoutMs, u as parseModelRef } from "./model-selection-jLPRV2hp.js";
...
import { d as resolveEnableState, f as resolveMemorySlotDecision, n as loadPluginManifestRegistry, u as normalizePluginsConfig } from "./manifest-registry-6Smo6hJJ.js";
import { d as TOOLS_BY_SENDER_KEY_TYPES, f as parseToolsBySenderTypedKey } from "./dock-CrncBkrf.js";
...
//#endregion
//#region src/config/prototype-keys.ts
...
function applyMergePatch(base, patch, options = {}) {
	...
	for (const [key, value] of Object.entries(patch)) {
		...
		if (isPlainObject$1(value)) {
			...
			result[key] = applyMergePatch(isPlainObject$1(baseValue) ? baseValue : {}, value, options);
			continue;
		}
		result[key] = value;
	}
...
function unwrapEnvInvocation(argv) {
	return scanWrapperInvocation(argv, {
		...
		onToken: (token, lower) => {
			...
			if (lower.startsWith("-u") || lower.startsWith("-c") || lower.startsWith("-s") || lower.startsWith("--unset=") || lower.startsWith("--chdir=") || lower.startsWith("--split-string=") || lower.startsWith("--default-signal=") || lower.startsWith("--ignore-signal=") || lower.startsWith("--block-signal=")) return "continue";
			return "invalid";
		}
	});
}
...
function unwrapStdbufInvocation(argv) {
	return scanWrapperInvocation(argv, {
		...
		onToken: (token, lower) => {
			if (!token.startsWith("-") || token === "-") return "stop";
			const [flag] = lower.split("=", 2);
			if (STDBUF_OPTIONS_WITH_VALUE.has(flag)) return lower.includes("=") ? "continue" : "consume-next";
			return "invalid";
		}
	});
}
...
function resolveCommandResolutionFromArgv(argv, cwd, env) {
	const rawExecutable = unwrapDispatchWrappersForResolution(argv)[0]?.trim();
	if (!rawExecutable) return null;
...
function renderSafeBinSegmentArgv(segment) {
	if (segment.argv.length === 0) return "";
	const resolvedExecutable = segment.resolution?.resolvedPath?.trim();
	return renderQuotedArgv(resolvedExecutable ? [resolvedExecutable, ...segment.argv.slice(1)] : segment.argv);
}
/**
...
function buildSafeBinsShellCommand(params) {
	...
	const rebuilt = rebuildShellCommandFromSource({
		...
		renderSegment: (raw, segmentIndex) => {
			...
			return {
				ok: true,
				rendered: by === "safeBins" ? renderSafeBinSegmentArgv(seg) : raw.trim()
			};
		}
	});
	if (!rebuilt.ok) return {
...
//#endregion
//#region src/infra/exec-safe-bin-policy.ts
...
function consumeLongOptionToken(args, index, flag, inlineValue, allowedValueFlags, deniedFlags) {
	if (deniedFlags.has(flag)) return -1;
	if (inlineValue !== void 0) return isSafeLiteralToken(inlineValue) ? index + 1 : -1;
	if (!allowedValueFlags.has(flag)) return index + 1;
...
function consumeShortOptionClusterToken(args, index, raw, cluster, flags, allowedValueFlags, deniedFlags) {
	for (let j = 0; j < flags.length; j += 1) {
		const flag = flags[j];
		if (deniedFlags.has(flag)) return -1;
		if (!allowedValueFlags.has(flag)) continue;
		const inlineValue = cluster.slice(j + 1);
		if (inlineValue) return isSafeLiteralToken(inlineValue) ? index + 1 : -1;
		return isInvalidValueToken(args[index + 1]) ? -1 : index + 2;
	}
...
function unsetPathForWrite(root, pathSegments) {
	...
	for (let i = 0; i < pathSegments.length - 1; i += 1) {
		...
		if (Array.isArray(cursor)) {
			...
			if (!Number.isFinite(index) || index < 0 || index >= cursor.length) return false;
			traversal.push({
				container: cursor,
				key: index
			});
		...
		if (!isWritePlainObject(cursor) || !(segment in cursor)) return false;
		traversal.push({
			container: cursor,
			key: segment
		});
	...
	if (Array.isArray(cursor)) {
		if (!isNumericPathSegment(leaf)) return false;
		const index = Number.parseInt(leaf, 10);
		if (!Number.isFinite(index) || index < 0 || index >= cursor.length) return false;
		cursor.splice(index, 1);
	} else {
		if (!isWritePlainObject(cursor) || !(leaf in cursor)) return false;
		delete cursor[leaf];
	}
	for (let i = traversal.length - 1; i >= 0; i -= 1) {
		const { container, key } = traversal[i];
		let child;
		if (Array.isArray(container)) child = typeof key === "number" ? container[key] : void 0;
		else if (isWritePlainObject(container)) child = container[String(key)];
		else break;
		if (!isWritePlainObject(child) || Object.keys(child).length > 0) break;
		if (Array.isArray(container) && typeof key === "number") {
			if (key >= 0 && key < container.length) container.splice(key, 1);
		} else if (isWritePlainObject(container)) delete container[String(key)];
	}
	return true;
}
...
function createConfigIO(overrides = {}) {
	...
	async function readConfigFileSnapshotInternal() {
		...
		try {
			...
			warnIfConfigFromFuture(validated.config, deps.logger);
			const snapshotConfig = normalizeConfigPaths(applyTalkApiKey(applyModelDefaults(applyAgentDefaults(applySessionDefaults(applyLoggingDefaults(applyMessageDefaults(validated.config)))))));
	...
	async function writeConfigFile(cfg, options = {}) {
		...
		const outputConfig = envRefMap && changedPaths ? restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) : cfgToWrite;
...
//#endregion
export { isWindowsPlatform as $, isWorkspaceRelativeAvatarPath as A, setConfigValueAtPath as B, parseDurationMs as C, resolveSubagentMaxConcurrent as Ct, isAvatarImageDataUrl as D, isAvatarHttpUrl as E, resetConfigOverrides as F, SAFE_BIN_PROFILES as G, getTrustedSafeBinDirs as H, setConfigOverride as I, validateSafeBinArgv as J, normalizeSafeBinProfileFixtures as K, unsetConfigOverride as L, resolveAvatarMime as M, validateJsonSchemaValue as N, isPathWithinRoot as O, getConfigOverrides as P, buildSafeShellCommand as Q, getConfigValueAtPath as R, sensitive as S, resolveAgentMaxConcurrent as St, isAvatarDataUrl as T, resolveOwnerDisplaySetting as Tt, isTrustedSafeBinPath as U, unsetConfigValueAtPath as V, normalizeTrustedSafeBinDirs as W, analyzeShellCommand as X, analyzeArgvCommand as Y, buildSafeBinsShellCommand as Z, isInboundPathAllowed as _, resolveSlackStreamingMode as _t, parseConfigJson5 as a, extractShellWrapperCommand as at, resolveIMessageRemoteAttachmentRoots as b, MAX_INCLUDE_DEPTH as bt, resolveConfigSnapshotHash as c, isShellWrapperExecutable as ct, OpenClawSchema as d, applyMergePatch as dt, splitCommandChain as et, parseByteSize as f, applyLegacyMigrations as ft, DEFAULT_IMESSAGE_ATTACHMENT_ROOTS as g, resolveSlackNativeStreaming as gt, resolveTelegramCustomCommands as h, resolveDiscordPreviewStreamMode as ht, loadConfig as i, resolveCommandResolutionFromArgv as it, looksLikeAvatarPath as j, isSupportedLocalAvatarExtension as k, writeConfigFile as l, unwrapKnownDispatchWrapperInvocation as lt, normalizeTelegramCommandName as m, mapStreamingModeToSlackLegacyDraftStreamMode as mt, migrateLegacyConfig as n, matchAllowlist as nt, readConfigFileSnapshot as o, extractShellWrapperInlineCommand as ot, TELEGRAM_COMMAND_NAME_PATTERN as p, isSafeExecutableValue as pt, resolveSafeBinProfiles as q, createConfigIO as r, resolveAllowlistCandidatePath as rt, readConfigFileSnapshotForWrite as s, isDispatchWrapperExecutable as st, config_exports as t, DEFAULT_SAFE_BINS as tt, validateConfigObjectWithPlugins as u, splitShellArgs as ut, mergeInboundPathRoots as v, resolveTelegramPreviewStreamMode as vt, AVATAR_MAX_BYTES as w, loadDotEnv as wt, normalizeScpRemoteHost as x, DEFAULT_SUBAGENT_MAX_SPAWN_DEPTH as xt, resolveIMessageAttachmentRoots as y, INCLUDE_KEY as yt, parseConfigPath as z };