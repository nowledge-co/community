...
- Stop declaring a bundled `127.0.0.1` MCP server from the plugin manifest. Kimi Code users should add the host-owned MCP block generated by `nmem config mcp show --host kimi-code`; this keeps remote/self-hosted Mem setups from seeing a duplicate broken local server while preserving skills, slash commands, and lifecycle thread capture.
- Pair with the matching Mem app/CLI release that trims padded `nmem t sync --from ...` host slugs, so hook payload/host formatting such as `kimi-code    ` resolves to `kimi-code`.
