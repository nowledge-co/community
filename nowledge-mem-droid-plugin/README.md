...
For multi-agent orchestrators, set `NMEM_AGENT_ID=<agent-slug>` per spawned Droid worker. Add `NMEM_SPACE` only when that run should override the AI Identity's default space. `NMEM_HOST_AGENT_ID` is for advanced external aliases. Context Bundle will use the stable identity while keeping `source_app=droid` for provenance.
