...
- If the runtime already knows the current project or agent lane, add `--space "<space name>"`. If a multi-agent orchestrator launched this Droid worker, set `NMEM_AGENT_ID="<agent-slug>"` before launch so Context Bundle resolves the right AI Identity. Use `NMEM_HOST_AGENT_ID` only for advanced host-id aliases.
