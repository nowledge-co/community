...
| **Paseo** | Configure the child runtime that Paseo launches | Multi-agent orchestration setup: install the connector for Codex, Claude Code, OpenCode, or Pi first; set `NMEM_AGENT_ID=<agent-slug>` only for durable Paseo agent roles. |
