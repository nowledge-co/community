...
| **Paseo** | Configure the child runtime that Paseo launches | Multi-agent orchestration setup: install the connector for Codex, Claude Code, OpenCode, or Pi first; use `nmem t sync --from paseo --all-projects --apply` to import supported child-session history from Paseo's registry; set `NMEM_AGENT_ID=<agent-slug>` only for durable Paseo agent roles. |
