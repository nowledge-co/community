...
| **[ZCode Plugin](nowledge-mem-zcode-plugin)** | Clone this repository, use the ZCode plugin README to create a persistent user-owned local marketplace catalog, then add that directory in **Settings → Plugins → Create → Add marketplace** and reload the Agent runtime | ZCode-native MCP + Skills package for guided Working Memory, proactive recall, distillation, status, and honest handoff summaries. It does not claim automatic transcript capture. |
