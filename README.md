...
| **Cumora** | Configure the child runtime or computer-level daemon | Daemon-boundary setup: connect Mem at the runtime/daemon boundary; use `NMEM_AGENT_ID` only if Cumora exposes per-agent runtime environment variables. |
