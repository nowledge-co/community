...
- Claude Code and Grok Build now save threads after each response in the background, so long sessions and temporary server failures no longer pause the conversation. Pre-compaction saves remain synchronous to protect context before it is discarded.
...
- Stop and PreCompact hooks now scan successfully saved transcripts for structured Nowledge Mem `find_skills` results and best-effort report `completed` outcomes with `nmem skills outcome`. Reports are deduplicated per transcript/skill/version so repeated hooks do not over-count the same skill use.
- Claude Code and Grok Build prompt nudges now mention managed skills: for recurring procedural work, check `find_skills` / `nmem skills match`, follow the matching `SKILL.md`, and report the outcome afterward.
...
- **Session capture no longer times out on large conversations.** The Stop and PreCompact save hooks used to cap each capture attempt at 8 seconds. On the current native (Rust) `nmem` CLI, saving a long conversation is a fresh process that parses the full transcript and writes it to the backend, which regularly needs more than 8 seconds. Every attempt then hit the cap and the hook reported "no flushed transcript found" even though nothing was actually wrong, so repeated saves appeared to silently fail. Capture now gives each attempt real time to finish and bounds the whole retry loop under the hook's overall limit, so long sessions save on the first try. Retries still ride out a backend that is briefly not ready right after a session starts.
