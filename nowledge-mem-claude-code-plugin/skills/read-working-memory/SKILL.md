...
> Start every session with context. Claude Code hooks prefer Context Bundle when available: owner identity, agent identity, active scope, guidance slots, and Working Memory. Working Memory alone is the lighter fallback.
...
If the runtime already knows the current project or agent lane, add `--space "<space name>"` to either command. Multi-agent orchestrators can set `NMEM_AGENT_ID`, `NMEM_HOST_AGENT_ID`, and `NMEM_SPACE` before launching Claude Code so hooks read the right Agent Identity automatically.
