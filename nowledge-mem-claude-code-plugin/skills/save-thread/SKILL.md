---
name: save-thread
description: Save the real Claude Code or Grok session messages only when the user explicitly requests it. Use nmem t save to import the recorded session, not a summary-only checkpoint.
---
...
Use `nmem t save` to import the current session. Use `claude-code` in Claude Code and `grok` in Grok:
...
**Options:**
- `--from`: Source app (`claude-code` for Claude Code, `grok` for Grok)
...
**Behavior:**
- Auto-detects sessions from `~/.claude/projects/` for Claude Code or `~/.grok/sessions/` for Grok
