...
If the runtime already knows the current project or agent lane, add `--space "<space name>"`. If the host has a stable long-running agent id, use `--host-agent-id "<agent-id>"` with `nmem context`.
