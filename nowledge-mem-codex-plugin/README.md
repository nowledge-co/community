...
For multi-agent orchestrators, set `NMEM_AGENT_ID` or `NMEM_HOST_AGENT_ID` per spawned Codex worker. Context Bundle will use that stable identity while keeping `source_app=codex` for provenance.
