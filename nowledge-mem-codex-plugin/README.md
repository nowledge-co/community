...
- **Codex threads are not appearing automatically**: rerun `scripts/install_hooks.py` from the installed plugin folder, then restart Codex. Current Codex loads automatic capture from `~/.codex/hooks.json`; newer builds can also load the bundled `hooks/hooks.json` when plugin hooks are enabled by the host.
