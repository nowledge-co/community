...
| **Codex CLI** | Running as Codex CLI agent; `~/.codex/` exists | Copy `nowledge-mem-codex-plugin` to `~/.codex/plugins/cache/local/nowledge-mem/local/`, then enable `[features] plugins = true` and `[plugins."nowledge-mem@local"] enabled = true` in `~/.codex/config.toml` | [Guide](https://mem.nowledge.co/docs/integrations/codex-cli) |
