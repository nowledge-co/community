...
| **Codex CLI** | Running as Codex CLI agent; `~/.codex/` exists | `codex plugin marketplace add nowledge-co/community` (legacy fallback: `codex marketplace add nowledge-co/community`), then enable `[features] plugins = true` and `[plugins."nowledge-mem@nowledge-community"] enabled = true` in `~/.codex/config.toml` | [Guide](https://mem.nowledge.co/docs/integrations/codex-cli) |
