...
| **OpenCode** | Running as OpenCode agent; `~/.config/opencode/` or `.opencode/` exists | `opencode plugin add opencode-nowledge-mem`; restart OpenCode so the event subscription and session hooks load | [Guide](https://mem.nowledge.co/docs/integrations/opencode) |
